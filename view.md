...
vec3 fade\_xyz = fade(Pf0);

vec4 n\_z = mix(vec4(n000,n100,n010,n110),vec4(n001,n101,n011,n111),fade\_xyz.z);

vec2 n\_yz = mix(n\_z.xy,n\_z.zw,fade\_xyz.y);

float n\_xyz = mix(n\_yz.x,n\_yz.y,fade\_xyz.x);
...
'#include <begin\_vertex>': `transformed.z += getPos(transformed.xyz);`,

'#include <beginnormal\_vertex>': `objectNormal = getNormal(position.xyz);`
...
float randomNoise = noise(gl\_FragCoord.xy);

gl\_FragColor.rgb -= randomNoise / 15. \* uNoiseIntensity;`
